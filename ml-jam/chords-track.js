class ChordsTrack {
  ...
  play_note(note, duration) {
    CHORDS_INSTRUMENT.triggerAttackRelease(note, duration);
    highlightKey("Chords" + note, "play");
  }
  
  add_note(time, note, duration) {
    this.melody.push({time : time, note : note, dur: duration});
    highlightKey("Chords" + note, "recorded");
  }
  
  start() {
    this.part = new Tone.Part(function(time, event){
      CHORDS_INSTRUMENT.triggerAttackRelease(event.note, event.dur, time);
      
      Tone.Draw.schedule(function() {
        highlightKey("Chords" + event.note, "recorded");
      }, time);
  ...
  set_volume(val) {
    CHORDS_INSTRUMENT.set("volume", val)
  }
}